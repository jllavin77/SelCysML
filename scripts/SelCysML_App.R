...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      fileInput("fasta_in", "Subir FASTA", accept = c(".fasta", ".fa")),
      checkboxInput("enable_blast", "Habilitar análisis funcional (BLAST)", value = TRUE),
      sliderInput("evalue_cut", "Umbral de E-value (log10):", min = -10, max = -1, value = -5, step = 1, pre = "1e"),
      helpText("Nota: El BLAST puede tardar varios minutos."),
      actionButton("predict_btn", "Ejecutar Diagnóstico", class = "btn-primary btn-block"),
      hr(),
      downloadButton("download_tsv", "Descargar Resultados (TSV)")
    ),
    mainPanel(
      tabsetPanel(
        tabPanel("Tabla de Predicción", DTOutput("results_table")),
        tabPanel("3D Prediction Space", 
                 plotlyOutput("pca_3d_plot", height = "600px"),
                 hr(),
                 downloadButton("down_pca_3d_html", "Exportar PCA 3D (HTML)"))
      )
    )
  )
)
...
server <- function(input, output) {
  ...
  results_data <- eventReactive(input$predict_btn, {
    req(input$fasta_in, models_bundle)
    raw <- readAAStringSet(input$fasta_in$datapath)
    
    # NORMALIZACIÓN INICIAL: Renombramos IDs antes de cualquier proceso
    names(raw) <- sapply(names(raw), clean_id_func)
    
    df_features <- extract_features_logic(as.character(raw))
    if(is.null(df_features)) stop("Error: No se pudieron extraer descriptores.")
    
    # Predicción
    preds_list <- map(models_bundle, ~predict(.x, df_features, type = "prob"))
    prob_selcys <- rowMeans(do.call(cbind, map(preds_list, ~ .x$.pred_SelCys)))
    
    # Construcción inicial usando nombres ya normalizados
    final_df <- data.frame(
      ID = names(raw)[1:nrow(df_features)],
  ...
  output$download_tsv <- downloadHandler(
    filename = "Resultados.tsv", content = function(file) { write.table(results_data() %>% select(-PC1, -PC2, -PC3), file, sep="\t", row.names=FALSE) }
  )
}
